...
uint8_t RemoteIP[] = {172,20,10,13};//{192,168,50,58};//{192,168,50,57};//{172,27,187,63};//{192,168,3,110};
...
int main(void)
{
  ...
  while(1)
  {
    if(Socket != -1)
    {
		...
		char buffer[64];  // Character buffer for formatted string
		sprintf(buffer, "X:%d Y:%d Z:%d\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
		ret = WIFI_SendData(Socket, (uint8_t *)buffer, strlen(buffer), &Datalen, WIFI_WRITE_TIMEOUT); // Send via TCP
